...
export function events(s){const services=list(s,'accounts').filter(r=>r.renewal&&r.status!=='cancelado'),renewals=services.map(r=>({...r,eventDate:r.renewal,name:`Renovación · ${r.name}`})),reminders=services.filter(r=>Number(r.reminderDays)>0).map(r=>{const date=new Date(r.renewal+'T12:00:00Z');date.setUTCDate(date.getUTCDate()-Number(r.reminderDays));return {...r,eventDate:date.toISOString().slice(0,10),name:`Revisar renovación · ${r.name}`};});return [...list(s,'tasks').filter(r=>r.date&&!['completada','cancelada'].includes(r.status)).map(r=>({...r,eventDate:r.date})),...list(s,'orders').filter(r=>r.due&&!['entregado','cancelado'].includes(r.status)).map(r=>({...r,eventDate:r.due,name:`Entrega · ${r.number}`})),...list(s,'content').filter(r=>r.date&&r.status!=='publicado').map(r=>({...r,eventDate:r.date})),...renewals,...reminders].sort((a,b)=>a.eventDate.localeCompare(b.eventDate));}
