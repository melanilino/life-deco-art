...
- La cabecera muestra «Vista actual» en lugar de «Crear». La etiqueta y el nombre de la vista comparten exactamente el mismo borde izquierdo, y la flecha aparece inmediatamente después del nombre. Cuando una sección tiene subcategorías, el selector permite cambiar entre ellas sin repetir pestañas; cuando tiene una sola vista, se muestra su nombre sin flecha. Las acciones para registrar elementos nuevos viven en la barra operativa de cada lista.
- En Clientes, «Nuevo cliente» aparece junto a «Exportar CSV» y abre el mismo panel lateral aprobado.
