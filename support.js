...
(() => {
  ...
  function showBootFallback() {
    if (!document.body || document.getElementById("dc-boot-fallback")) return;
    // A tiny brand shell guarantees a contentful first paint while the runtime
    // loads. It matches the real fixed navigation and disappears after mount.
    const el = document.createElement("div");
    el.id = "dc-boot-fallback";
    el.setAttribute("aria-hidden", "true");
    el.style.cssText = "position:fixed;top:0;left:0;right:0;height:80px;z-index:2147482999;background:#fff;display:flex;align-items:center;padding:0 48px;box-sizing:border-box;font-family:Montserrat,Arial,sans-serif;color:#000";
    el.innerHTML = '<span style="font-size:20px;font-weight:400;letter-spacing:.13px;white-space:nowrap">Life Deco Art</span>';
  ...
  function removeBootFallback() {
    const el = document.getElementById("dc-boot-fallback");
    if (el) el.remove();
  }
  ...
  function loadScript(src, integrity, timeoutMs = 5000) {
    return new Promise((resolve2, reject) => {
      ...
      s.async = false;
      try { s.fetchPriority = "high"; } catch {}
      s.onload = () => finish();
  ...
  function loadWithFallback(primary, fallback, integrity) {
    return loadScript(primary, integrity, 4500).catch(() =>
      loadScript(fallback, integrity, 6500)
    );
  }
  ...
  const ensureFallback = () => {
    if (document.querySelector("x-dc")) showBootFallback();
  };
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", ensureFallback, { once: true });
  else ensureFallback();
  loadReactUmd().then(init).catch((err) => {
    // Keep the first-paint shell visible if both CDNs fail. This is preferable
    // to a completely blank document and makes failures observable.
    showBootFallback();
